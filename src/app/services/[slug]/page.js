    ...
    <main className="min-h-screen bg-ivory-sacred">
      ...
      <section className="pt-36 pb-20 bg-indigo-deep text-white relative">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <span className="font-decorative text-xs text-gold-primary tracking-widest uppercase block mb-2">
            Service Specification
          </span>
          <h1 className="font-serif text-3xl sm:text-5xl text-white mb-4">{data.title}</h1>
          <p className="font-sans italic text-ivory-sacred/80 text-base max-w-2xl font-light">
            "{data.subtitle}"
          </p>
        </div>
      </section>
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="bg-white border border-gold-border rounded-xl p-8 sm:p-10 shadow-floating">
              <h2 className="font-serif text-2xl text-indigo-dark mb-4">Service Capabilities</h2>
              <p className="font-sans text-charcoal/80 text-base leading-relaxed">{data.overview}</p>
            </div>

            {/* Feature Breakdown */}
            <div className="space-y-6">
              <h2 className="font-serif text-2xl text-indigo-dark">Key Solutions & Modules</h2>
              <div className="grid gap-6">
                {data.features.map((item) => (
                  <div
                    key={item.title}
                    className="bg-white border border-gold-border/40 hover:border-gold-primary rounded-xl p-6 sm:p-8 shadow-floating transition-all"
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-8 h-8 rounded-full bg-gold-primary/10 border border-gold-primary/30 flex items-center justify-center text-gold-dark font-bold text-xs">
                        ✓
                      </div>
                      <h3 className="font-serif text-xl text-indigo-dark">{item.title}</h3>
                    </div>
                    <p className="font-sans text-xs text-charcoal/70 leading-relaxed pl-11">
                      {item.desc}
                    </p>
                  </div>
                ))}
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-28 bg-emerald-dark text-white border border-gold-border rounded-xl p-8 shadow-2xl">
              ...
              <h3 className="font-serif text-2xl text-gold-primary mb-4">Request Customized Logistics</h3>
